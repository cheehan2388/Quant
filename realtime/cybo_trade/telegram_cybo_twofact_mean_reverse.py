...
class Strategy(BaseStrategy):
    ...
    bot_id = "[btc_funding.oi_ratio_threshold_1m]"
    total_pnl = 0.0
    ...
    def __init__(self):
        ...
        file_handler = TimedRotatingFileHandler(
            "y_funding.oi_ratio_MR_1m_threshold.log", when="h", backupCount=30
        )
    ...
    async def on_datasource_interval(self, strategy, topic, data_list):
        ...
        if len(self.processed_data) == len(datasource_topics):
            ...
            min_rat = min(ratio.rolling(windows = self.rolling_window))
            max_rat = max(ratio.rolling(windows = self.rolling_window))
            
            normalize = ratio[-1] - min_rat/(max_rat -  min_rat)
            ...
            qty = self.qty
            #Min_max Mean_Reverse
            #empty pos with >=  threshold short
            if self.position.short.quantity == 0.0 and self.position.short.quantity == 0  and normalize >= self.multiplier:
                ...
                try:
                    ...
                    logging.info(
                        f"Start with a short order with qty {qty} when current_price: {current_price}, ratio: {ratio[-1]}, normalize: {normalize}at {util.convert_ms_to_datetime(start_time[-1])}"
                    )
                    self.position = Position(
                        self.pair,
                        PositionData(quantity=0.0, avg_price=0),
                        PositionData(quantity=qty, avg_price=current_price),
                        updated_time = time
                    )
                    self.entry_time = util.convert_ms_to_datetime(start_time[-1])
                    mess = (
                        f"{self.bot_id} bot <u>opened a SHORT position</u>\n"
                        f"<b>quantity</b>: {qty}\n"
                        f"<b>current_price</b>: {current_price}\n"
                        f"<b>ratio</b>: {ratio}\n"
                        f"<b>normalize</b>: {round(normalize, 2)}\n"
            ...
            elif   normalize < 0.0 and  self.position.short.quantity != 0.0:
                
                    try:
                        await strategy.open(
                            side=OrderSide.Buy,
                            quantity=self.position.short.quantity,
                            take_profit=None,
                            stop_loss=None,
                            symbol=self.pair,
                            exchange=Exchange.BybitLinear,
                        ...
                        self.total_pnl += pnl
                        logging.info(
                            f"Closed Short position with qty {1*self.position.short.quantity}, pnl: {pnl}, total_pnl: {self.total_pnl} when current_price: {current_price}, ratio: {ratio[-1]}, normalize: {normalize}, sma_with_multiplier at {util.convert_ms_to_datetime(start_time[-1])}"
                        )
                        ...
                        mess = (
                            ...
                            f"<b>quantity</b>: {abs(self.position.short.quantity*2)}\n"
                            f"<b>pnl</b>: {pnl}\n"
            ...
            elif   normalize <= -self.multiplier and  self.position.long.quantity == 0.0 and self.position.short.quantity == 0.0:
                    ...
                    try:
                        ...
                        logging.info(
                            f"Start with a buy position with qty {qty},   when current_price: {current_price}, ratio: {ratio[-1]}, normalize: {normalize}, sma_with_multiplier at {util.convert_ms_to_datetime(start_time[-1])}"
                        )
                        ...
                        mess = (
                            f"{self.bot_id} bot <u>closed a LONG position</u>\n"
                            f"<b>quantity</b>: {abs(self.position.long.quantity)}\n"
                            f"<b>pnl</b>: {pnl}\n"
                            f"<b>total_pnl</b>: {self.total_pnl}\n"
                            f"<b>current_price</b>: {current_price}\n"
                            f"<b>ratio</b>: {ratio}\n"
            ...
            elif  normalize > 0 and  self.position.long.quantity != 0.0:
                    ...
                    try:
                        ...
                        self.total_pnl += pnl
                        logging.info(
                            f"Closed a buy position with qty {self.position.long.quantity}, pnl: {pnl}, total_pnl: {self.total_pnl} when current_price: {current_price}, ratio: {ratio[-1]}, normaliz: {normalize}, sma_with_multiplier at {util.convert_ms_to_datetime(start_time[-1])}"
                        )
...
config = RuntimeConfig(
    ...
    candle_topics=["binance-linear|candle?interval=1m&symbol=BTCUSDT"],
