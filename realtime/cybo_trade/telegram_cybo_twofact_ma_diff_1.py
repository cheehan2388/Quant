...
class Strategy(BaseStrategy):
    ...
    async def on_datasource_interval(self, strategy, topic, data_list):
        ...
        if len(self.processed_data) == len(datasource_topics):
            ...
            qty = self.qty
            double_qt = self. qty_doub
            if self.position.long.quantity == 0.0 and self.position.short.quantity == 0 and dif >= self.multiplier:
                ...
                try:
                    ...
                    logging.info(
                        f"Placed a 1st order (SHORT) with qty {qty} when current_price: {current_price}, ratio: {ratio[-1]}, dif: {dif[-1]}at {util.convert_ms_to_datetime(start_time[-1])}"
                    )
            ...
            elif  dif <= -self.multiplier and  self.position.short.quantity != 0.0:
                    ...
                    try:
                        ...
                        self.total_pnl += pnl
                        logging.info(
                            f"Closed a Short position with qty {self.position.short.quantity} and start a Buy position with qty {qty}, pnl: {pnl}, total_pnl: {self.total_pnl} when current_price: {current_price}, ratio: {ratio[-1]}, dif: {dif}, sma_with_multiplier at {util.convert_ms_to_datetime(start_time[-1])}"
                        )
            ...
            elif dif < - self.multiplier and self.position.short.quantity == 0 and self.position.long.quantity == 0:
                    ...
                    try:
                        ...
                        self.total_pnl += pnl
                        logging.info(
                            f"Open Buy position with qty {qty},  when current_price: {current_price}, ratio: {ratio[-1]}, dif : {dif}, sma_with_multiplier at {util.convert_ms_to_datetime(start_time[-1])}"
                        )
            ...
            elif self.position.long.quantity != 0.0 and dif >= self.multiplier:
                ...
                try:
                    ...
                    logging.info(
                        f"Close  a buy order with qty {self.position.long.quantity} and Open a Short order with qty {qty} when current_price: {current_price}, ratio: {ratio[-1]}, dif: {dif}at {util.convert_ms_to_datetime(start_time[-1])}"
                    )
                    self.position = Position(
                        self.pair,
