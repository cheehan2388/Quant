...
date_col      = 'time'
inter = 'day' # give an initial inter 
price = 'Close'
def delay_test(positions,price, strat, sr_multi):
    
    price = pd.read_csv("./../Data/Binance_1m_BTCUSDT_2020-01-01_2025-07-07.csv")
...
def load_and_split_data(
    ...
    # df_back = df.loc[:back_lens].copy()
    # df_val  = df.loc[back_lens:val_lens].copy() 
    # df_fwd  = df.loc[back_lens + val_lens :]
    ...
    df_fwd  = df.loc[back_end:forward_end].copy()
    return df_back, df_fwd



def plot_parameter_heatmaps(df, model, strategy, output_dir, interval):
...
def permutation_test(backtested_df, row ,strat_funcs :dict, models_funcs : dict ,column_name : str, num_perm = 500, seed = 20,sr_multiplier = None ): 
    ...
    daily_change = backtested_df[price].pct_change().fillna(0)
...
def run_pipeline(data_file, output_dir:str,
    ...
    price_back  = df_back[price]
    price_fwd   = df_fwd[price]
    ...
    mm_windows  = np.arange(20, 500, 10)
    ...
    thr_zscore   = np.arange(0.0, 3.0, 0.5)
        ...
        for param in param_list:
            ...
            for strat, strat_fn in strat_funcs.items(): # mr(strat) : func()(strat_fn)
                for thr in thr_list:
                    ...
                    full_date_stats = backtest(df[price],positions= pos_all,sr_multiplier = sr_multiplier,fee = fee) 
                    results.append({
    ...
    with open(os.path.join(output_dir, f'{column_name}_{interval}.json'), "w") as f:
    ...
    if heatmap == True:
       ...
       for f in df_all['model'].unique():
            for s in df_all[df_all['model']==f]['strategy'].unique():
                plot_parameter_heatmaps(df_all, f, s, output_dir, interval)
    ...
    for idx, row in df_res.iterrows():
        ...
        if pval < 0.05:
            ...
            bt_full = backtest(df_full[price], pos_full,fee = fee, sr_multiplier=sr_multiplier)            
...
if __name__ == '__main__':
    DATA_FILE = '../Data/.csv'
    run_pipeline(DATA_FILE, output_dir='results', interval='1h', column_name='data',sr_multiplier = 24, fee =0.0006)
