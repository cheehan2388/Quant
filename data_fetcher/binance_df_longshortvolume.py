...
# ────── 基本參數 ────────────────────────────────────────────────────────────────
data = "takerlongshortRatio"
API_ENDPOINT = 'https://dapi.binance.com/futures/data/takerBuySellVol'
OUTPUT_DIR   = "./../Data"              # 依需求修改
LIMIT        = 500                     # Binance 最大值
SYMBOLS      = ["BTC", "ETH", "BNB", "LINK", "AVAX",
                "XRP", "DOGE", "SUI", "BCH", "SOL"]
contract     = "PERPETUAL"
INTERVALS    = [ "5m","1h"]             # 想加別的就放進來
...
COLS_LONGSHORT = ["takerBuyVol","takerSellVol", "takerBuyVolValue", "takerSellVolValue","timestamp" ]

# ────── 工具函式 ────────────────────────────────────────────────────────────────
def fetch_long_short_rat(symbol: str, interval: str,
    ...
    df = pd.DataFrame(all_rows, columns= COLS_LONGSHORT)
    if df.empty:
        return df
...
def main():
    ...
    for base in SYMBOLS:
        ...
        for interval in INTERVALS:
            ...
            df = fetch_long_short_rat(symbol, interval, START_TIME, END_TIME)
            if df.empty:
                logging.warning(f"No data retrieved for {symbol} {interval}")
                continue

            fname = (f"Binance_{interval}_{symbol}_{data}"
