...
def load_data(file_path='../Data/merged/BTC_merged.csv'):
...
def select_important_features(df, target='forward_return', top_n=100, importance_threshold=0.01): # top 100 選取前100的feature
...
def compute_sum_ic(df, feature_col, close_dir):
    ...
    for close_path in close_files:
        ...
        merged['forward_return'] = merged['Close'].pct_change().rolling(24,period = 24).mean().shift(-24)
