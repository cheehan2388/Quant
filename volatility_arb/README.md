# Volatility Arbitrage (VRP) Scaffold

This module provides a practical scaffold for a variance risk premium (VRP) based volatility arbitrage strategy on crypto (e.g., BTC):

- Forecast realized volatility using HAR-RV on daily realized variance inferred from intraday returns
- Compare implied volatility to the forecast; form a VRP signal
- Simulate a variance-swap-like PnL from vega-targeted exposure, with simple costs

## Files
- `har_rv.py`: Realized variance and HAR-RV forecast utilities
- `signals.py`: VRP signal construction and vega sizing with simple risk filters
- `execution.py`: Simplified variance-swap-like PnL simulator
...
1) Prices CSV (intraday)
```
datetime,price
2024-01-01 00:00:00,42500
2024-01-01 00:05:00,42510
...
```

2) Implied vol CSV (daily, annualized; optional)
```
datetime,iv
2024-01-01,0.60
2024-01-02,0.58
...
Run:
```
python -m Quant.volatility_arb.backtest --prices_csv path/to/prices.csv --iv_csv path/to/iv.csv --out_csv results.csv
```

If `--iv_csv` is omitted, a naive proxy is used.

## Notes
- This is a scaffold, not a full option greeks engine. It approximates variance exposure and costs.
- Extend with real option chain fetching (e.g., Deribit) and explicit straddle construction, delta hedging, and risk overlays.
